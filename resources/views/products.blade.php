...
@section('subheader-buttons')
    ...
    <div class="k-subheader__toolbar">
        <div class="k-subheader__wrapper">
            ...
            <a href="javascript:void(0);" class="btn btn-label btn-label-brand btn-bold" title="Edit" data-placement="top" onClick="DevPos.Product.create();">
                <i class="fa fa-plus"></i> <span class="d-none d-md-inline"> {{ __('Product') }}</span>
            </a>
...
@section('js-local')
    ...
    <script>
        ...
        Cache.put('products', @php echo json_encode($items) @endphp);
        Cache.put('categories', @php echo json_encode($categories) @endphp);
        Cache.put('suppliers', @php echo json_encode($suppliers) @endphp);
        Cache.put('units', @php echo json_encode($units) @endphp);
